...
endmethod.
  method check_required_fields.
    ...
    lt_required = value #(
      ( field_name = 'gv_proc_type'   label = 'İşlem Türü' )
      ( field_name = 'gv_comp_code'   label = 'Şirket Kodu' )
      ( field_name = 'gv_doc_date'    label = 'Belge Tarihi' )
      ( field_name = 'gv_snd_acc_no'  label = 'Gönderen Hesap No' )
      ( field_name = 'gv_amount'      label = 'Tutar' )
      ( field_name = 'gv_rvc_acc_no'  label = 'Alıcı Hesap No' )
      ( field_name = 'gv_rvc_amount'  label = 'Alıcı Tutar' )
      ( field_name = 'gv_rvc_iban'    label = 'Alıcı IBAN' )
      ( field_name = 'gv_rvc_pb'      label = 'Alıcı Para Birimi' )
    ).

    " Dinamik olarak her alanı kontrol et
  ...
  method check_required_fields_fx.
    ...
    lt_required = value #(
      ( field_name = 'gv_fx_bank'           label = 'Banka Kodu' )
      ( field_name = 'gv_fx_iban'           label = 'IBAN' )
      ( field_name = 'gv_fx_acc_no'         label = 'Hesap Numarası' )
      ( field_name = 'gv_fx_amount'         label = 'Tutar' )
      ( field_name = 'gv_fx_amount_trans'   label = 'Çevrilmiş Tutar' )
      ( field_name = 'gv_fx_pb'             label = 'Kaynak Para Birimi' )
      ( field_name = 'gv_fx_pb_trans'       label = 'Hedef Para Birimi' )
      ( field_name = 'gv_fx_valid_date'     label = 'Geçerlilik Tarihi' )
      ( field_name = 'gv_fx_doc_date'       label = 'Belge Tarihi' )
      ( field_name = 'gv_fx_upb_amount'     label = 'TRY Karşılığı' )
      ( field_name = 'gv_fx_curr_type'      label = 'Kur Tipi' )
      ( field_name = 'gv_fx_curr'           label = 'Kur' )
    ).
  ...
  method prepare_alv.
    ...
    " 1. Alanlar kontrol edilir
    if gv_proc_type = '5'.
      lv_valid = me->check_required_fields_fx( ).
      if lv_valid = abap_true.
        me->save_data( ). " sadece doluysa kayıt atılır
      endif.
    else.
    ...
    " 2. Her durumda eski kayıtlar gösterilir
    me->select( ).
    me->create_alv( ).
  ...
  method handle_save.
    ...
    " Mesaj oluştur
    if gv_proc_type = '5'.
      concatenate lv_rec_id_t002 'numaralı talimatı yazdırmak ister misiniz?' into lv_question separated by space.
    else.
      concatenate lv_rec_id_t001 'numaralı talimatı yazdırmak ister misiniz?' into lv_question separated by space.
    endif.

    " 1. Popup
    call function 'POPUP_TO_CONFIRM'
      exporting
        titlebar              = 'Talimat Yazdırma'
    ...
    if lv_answer1 = '1'.
      call function 'POPUP_TO_CONFIRM'
        exporting
          titlebar              = 'Emin Misiniz?'
          text_question         = 'Yazdırma işlemine devam etmek istiyor musunuz?'
          text_button_1         = 'Evet'
          text_button_2         = 'Hayır'
          default_button        = '2'
          display_cancel_button = ''
